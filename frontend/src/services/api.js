...
const api = axios.create({
  ...
  timeout: 300000, // Увеличиваем таймаут до 5 минут
  headers: {
...
api.interceptors.response.use(
  response => response,
  error => {
    // Если запрос не прошел из-за CORS, пробуем с другими настройками
...
export const getSummaryRange = async (startDate, endDate, progressCallback = null) => {
  ...
  return getCachedData(cacheKey, async (progressMonitor) => {
    try {
      // Использовать оптимизированный API-метод для получения данных за весь период сразу
...
export const getCountrySummaryRange = async (startDate, endDate, progressCallback = null) => {
  ...
  return getCachedData(cacheKey, async (progressMonitor) => {
    try {
      // Используем прямой запрос для получения данных за весь период сразу
      console.log(`Загрузка данных по странам за период ${startDate} - ${endDate}`);
      if (progressMonitor) progressMonitor(10, 0); // Начальный прогресс
      
      // Пробуем запросить все данные сразу
      const response = await api.get(`/country_range_summary?start_date=${startDate}&end_date=${endDate}`);
      ...
      // Сначала пробуем загрузить по месяцам
      try {
        const start = new Date(startDate);
        const end = new Date(endDate);
        ...
        for (let i = 0; i < months.length; i++) {
          ...
          if (progressMonitor && totalMonths > 0) {
            // Безопасно вычисляем процент прогресса
